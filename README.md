...
> _**Note on API Quota Costs:** Most tools are highly efficient, costing only **1 unit** per call. The exceptions are the search-based tools: `searchVideos` costs **100 units** and `getChannelTopVideos` costs **101 units**. The `getTranscripts` tool has **0** API cost. The `findConsistentOutlierChannels` tool is a high-cost discovery tool that performs multiple API calls for comprehensive analysis. The `getPlaylistAnalytics` tool costs **1 unit** per call._
